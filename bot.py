...
class Agent:
    ...
    def parse_board(self):
        if self.game.g_count > 0:
            ...
            self.g_letters = [
                l
                for l in self.g_letters
                if l not in self.y_letters and l not in self.prediction
            ]
    ...
    def choose_action(self):
        ...
        if len(self.g_letters) > 0:
            self.w_bank = self.w_bank[
                ~self.w_bank["words"].str.contains("|".join(self.g_letters))
            ]
            self.g_letters = []
        if len(self.y_letters) > 0:
            y_str = "^" + "".join(rf"(?=.*{l})" for l in self.y_letters)
        ...
        mv_bank = self.w_bank[
            self.w_bank["w-score"] == self.w_bank["w-score"].max()
        ]
